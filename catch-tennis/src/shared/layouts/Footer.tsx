...
export const Footer = () => {
    ...
    return (
        <footer className="sticky bottom-0 z-50 border-t border-border/60 bg-surface/95
            px-4 pt-xs pb-0 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur">
            <nav className="mx-auto flex max-w-[480px] items-center justify-between gap-4">
                {navElems.map((navElem) => {
                    ...
                    return (
                        <button
                            type="button"
                            className="flex flex-1 flex-col items-center gap-1 rounded-2xl px-2 text-xs font-medium transition-all"
                            onClick={() => {navigate(`${navElem.dest}`)}}
                            key={navElem.imgType}
                            aria-current={active ? 'page' : undefined}
                        >
                            <span className={`flex w-full items-center justify-center rounded-2xl border text-text-muted transition-colors ${active ? 'border-primary/40 bg-primary/10 text-primary' : 'border-transparent hover:text-text-title'}`}>
                                <IconLoader
